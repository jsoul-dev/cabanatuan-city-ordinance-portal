...
export default async function OrdinanceDetailPage({
  ...
  return (
    <>
      ...
      <main
        id="main-content"
        className="flex-1 py-8 bg-[#070b09] text-white transition-colors duration-300 min-h-screen"
      >
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          {/* Breadcrumb */}
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex items-center space-x-2 text-xs text-neutral-400">
              <li>
                <Link
                  href="/"
              ...
              <li aria-hidden="true">/</li>
              <li className="font-semibold text-white truncate max-w-[220px] sm:max-w-xs">
          ...
          {/* Hero Header Card */}
          <div className="mb-8 rounded-2xl border border-neutral-800 bg-[#0d1310] p-5 sm:p-7 shadow-xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
              ...
              <span className="text-sm font-mono font-bold text-neutral-300 bg-neutral-900 px-3.5 py-1 rounded-md border border-neutral-800">
                Res. No. {formatResolutionDisplay(ord.resolutionNumber)}
              </span>
            </div>

            {/* Title */}
            <h1 className="text-2xl sm:text-3xl font-black text-white mb-4 tracking-tight leading-snug">
              {cleanTitle}
            </h1>

            {/* Executive Summary Box (Green Left Border) */}
            {ord.description && (
              <div className="mb-5 rounded-r-xl border-l-4 border-emerald-500 bg-emerald-500/10 p-4 text-sm sm:text-base text-neutral-200 leading-relaxed font-normal">
                {ord.description}
              </div>
            )}
            ...
            {ord.ordinanceLabel && (
              <div className="mb-4 inline-flex items-center gap-2 rounded-md bg-neutral-800/80 border border-neutral-700/60 px-3 py-1.5 text-xs font-mono font-semibold text-neutral-300">
            ...
            {/* Metadata Bar */}
            <div className="flex flex-wrap items-center gap-5 text-xs sm:text-sm text-neutral-400 mb-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-white">{coverageText}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-5">
              {ord.category && (
                <span className="inline-flex items-center rounded-md bg-neutral-800 border border-neutral-700/80 px-3 py-1.5 text-xs font-extrabold uppercase tracking-wider text-neutral-200">
                  {ord.category}
                </span>
              )}
              {ord.type === "BARANGAY" && (
              ...
              {ord.tags &&
                ord.tags.map((tag, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center rounded-md bg-neutral-800 border border-neutral-700/80 px-3 py-1.5 text-xs font-extrabold uppercase tracking-wider text-neutral-200"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div className="mt-10 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center shadow-lg">
            ...
            <p className="text-sm text-neutral-300 max-w-xl mx-auto mb-5">
